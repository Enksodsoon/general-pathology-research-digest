...
def score_paper(paper: Paper, config: Dict) -> Paper:
    haystack = f"{paper.title} {paper.abstract} {paper.publication_type} {paper.category}".lower()
    matched_profiles: List[str] = []
    relevance = 0

    for profile in config.get("profiles", []):
        hits = [term for term in profile.get("terms", []) if term.lower() in haystack]
        if hits:
...
def decide(paper: Paper, config: Dict) -> str:
    ...
    if LEGACY_PROFILE in profiles and not (PRIMARY_PATHOLOGY_PROFILES & profiles):
