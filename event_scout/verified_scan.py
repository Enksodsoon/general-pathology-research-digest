...
def discover(source,now):
    started=time.monotonic(); url=source['url'].replace('{date}',now.date().isoformat()).replace('{year}',str(now.year)); rows=[]
    if 'query' in source:
        q=source['query'].replace('{year}',str(now.year)).replace('{thai_year}',str(now.year+543))
        url='https://www.bing.com/search?'+urlencode({'q':q,'format':'rss','setlang':source['language']})
    try:
        html,final=fetch(url)
        if source.get('kind')=='search':
            rss=ElementTree.fromstring(html)
            for item in rss.findall('.//item'):
                if item.findtext('link'): rows.append((item.findtext('link'),item.findtext('title') or ''))
            ...
            for a in soup.select('a[href]'):
                ...
                if re.search(r'webinar|seminar|/event|conference|sympos|อบรม|ประชุม|สัมมนา|セミナー|ウェビナー|\.html',href+' '+label,re.I):rows.append((href,label))
            rows.insert(0,(final,''))
        output=[]
        for href,label in rows:
            ...
            if p.hostname=='news.google.com':continue
            row=dict(source,url=clean_url(href),label=label,discovered_by=source['id'])
            if source.get('kind')=='search':
                row['timezone']='';row['medical']=False
            output.append(row)
        output.sort(key=lambda r:-bool(re.search(r'webinar|seminar|ประชุม|อบรม|セミナー|ウェビナー',r.get('label',''),re.I)))
        return output[:source.get('max_links',45)],{'id':source['id'],'language':source['language'],'url':url,'status':'ok','candidates':len(output),'seconds':round(time.monotonic()-started,2)}
